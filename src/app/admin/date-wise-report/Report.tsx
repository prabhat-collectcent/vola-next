...
export default function ReportPage() {
  ...
  useEffect(() => {
    async function fetchReports() {
      ...
      try {
        ...
        const fetchedReports = await getDateWiseReportsAction(queryParams);
        if (fetchedReports.success) {
          console.log('fetchedReports', fetchedReports);
          // @ts-ignore
          setData(
            fetchedReports.data.map((item: any) => ({
              id: item.id,
              ...
              impressions: '-',
              sales: 0,
              google_clicks: item.google_clicks,
              //   adobe_visits: item.adobe_visits,
              //   domestic_bookings: item.domestic_bookings,
              //   international_bookings: item.international_bookings,
              //   booking_total: item.total_bookings,
            }))
          );
        } else {
          showToast(
  ...
  const allColumns = [
    ...
    { key: 'google_clicks', label: 'Clicks' },
        { key: 'sales', label: 'Sales' },

    { key: 'impressions', label: 'CR%' },
    // { key: 'adobe_visits', label: 'Visits on Adobe - Airindia' },
    // { key: 'domestic_bookings', label: 'Bookings - Domestics' },
    // { key: 'international_bookings', label: 'Bookings - International' },
