...
export default function LocationSearchBox({
  ...
  const [foundLocations, setFoundLocations] = useState<
    { id: string; resourceName: string; canonicalName: string }[]
  >([]);
 ...
 useEffect(() => {
  ...
  async function fetchLocations() {
    ...
    const res = await searchLocationAction(currentQuery, {
  ...
  const addLocation = (loc: Location) => {
    const foundLoc = selected.find(
      (l) => l.geoTargetConstant === loc.resourceName
    );
  ...
  const removeLocation = (loc: Location) => {
    dispatch({
      type: 'SET_FIELD',
      payload: {
        [type === 'include' ? 'geo_include' : 'geo_exclude']: selected.filter(
          (l: any) => {
            if (loc.resourceName) {
  ...
  const handleCSVUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const unique = parsedLocations.filter(
        (csvLoc) =>
          !selected.some(
            (s) =>
    ...
    <div className="mb-6">
      ...
      <div ref={wrapperRef} className="relative">
        ...
        <div className="flex items-center gap-1 mb-2 mt-4">
          <label className="text-[13px] text-[#1E1E1E]">Chose file</label>
